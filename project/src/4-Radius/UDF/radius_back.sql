...
BEGIN	
	...
	CREATE TABLE IF NOT EXISTS bit_str(node bigint, str bigint[16]);
	FOR i IN 1..n LOOP
		FOR j IN 1..16 LOOP
	...
	CREATE INDEX id1 ON bit_str(node);
	CREATE INDEX id2 ON adj_lst(n_from);
	CREATE INDEX id3 ON adj_lst(n_to);
	...
	FOR i IN 1..iter LOOP
 	...
 	-- 		WHERE bit_str_check.node=bit_str.node and bit_str_check.str=bit_str.str)' using round;
